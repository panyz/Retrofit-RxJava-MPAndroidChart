...
public class CandleDataSet extends LineScatterCandleRadarDataSet<CandleEntry>
        implements com.jn.chart.interfaces.datasets.ICandleDataSet {
     ...
     */
    protected int mNeutralColor = com.jn.chart.utils.ColorTemplate.COLOR_NONE;

    /**
     * color for open < close
     */
    protected int mIncreasingColor = com.jn.chart.utils.ColorTemplate.COLOR_NONE;

    /**
     * color for open > close
     */
    protected int mDecreasingColor = com.jn.chart.utils.ColorTemplate.COLOR_NONE;

    /**
     * shadow line color, set -1 for backward compatibility and uses default
     * color
     */
    protected int mShadowColor = com.jn.chart.utils.ColorTemplate.COLOR_NONE;
    ...
    @Override
    public DataSet<com.jn.chart.data.CandleEntry> copy() {

        List<com.jn.chart.data.CandleEntry> yVals = new ArrayList<CandleEntry>();

        for (int i = 0; i < mYVals.size(); i++) {
            yVals.add(((com.jn.chart.data.CandleEntry) mYVals.get(i)).copy());
        }
    ...
    public void calcMinMax(int start, int end) {
        ...
        for (int i = start; i <= endValue; i++) {
            ...
            com.jn.chart.data.CandleEntry e = mYVals.get(i);
    ...
    public void setShadowWidth(float width) {
        mShadowWidth = com.jn.chart.utils.Utils.convertDpToPixel(width);
    }
